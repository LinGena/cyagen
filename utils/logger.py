...
class Logger():
    ...
    def _init_logger(self):
        self._log_format = logging.Formatter(os.getenv("LOGS_FORMAT"))
        self._init_console_logger()
        self._init_file_logger(file_name=self._file_name)
        self._logger.setLevel(os.getenv("LOGS_LEVEL"))
    ...
    def _get_log_path(self, file_name: str) -> str:
        log_path = f"{os.getenv('LOGS_DIR')}/{self._ROLLOVER_SUFFIX}/{file_name}.log"
        return log_path